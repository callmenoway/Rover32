...
static const int posRight = 30;
static const int posLeft = 30;
static const int posRightLittle = 10;
static const int posLeftLittle = 10;
...
void handleWebSocketEvent(uint8_t num, WStype_t type, uint8_t *payload, size_t length) {
  Serial.println("WebSocket event received");
      const char *message = reinterpret_cast<const char *>(payload);  // Cast payload to const char*
  ...
  switch (type) {
    ...
    case WStype_TEXT:
      ...
      if (strcmp(message, "forward") == 0) {
        ...
        digitalWrite(motor1Pin1, LOW);
        digitalWrite(motor1Pin2, LOW);
      } else if (strcmp(message, "creepForward") == 0) {
        ledcWrite(pwmChannel, dutyCycle);
      } else if (strcmp(message, "right") == 0) {
        steering.write(posRight);
      } else if (strcmp(message, "left") == 0) {
        steering.write(posLeft);

      } else if (strcmp(message, "leftLittle") == 0) {
        steering.write(posLeftLittle);

      } else if (strcmp(message, "rightLittle") == 0) {
        steering.write(posRightLittle);
      }
      break;
    case WStype_BIN:
